...
export async function GET(request: Request) {
  ...
  try {
    ...
    const pagePerformanceData = response.data.rows?.map((row: any) => ({
      page: row.keys[0],
      clicks: row.clicks,
    ...
    return NextResponse.json(pagePerformanceData);
  } catch (error) {
    log('Error fetching page performance data:', error);
    return NextResponse.json({ error: 'Failed to fetch page performance data' }, { status: 500 });
  }
}