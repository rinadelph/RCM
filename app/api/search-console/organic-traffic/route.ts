...
export async function GET(request: Request) {
  ...
  try {
    ...
    const organicTrafficData = response.data.rows?.map((row: any) => ({
      date: row.keys[0],
      clicks: row.clicks,
    ...
    return NextResponse.json(organicTrafficData);
  } catch (error) {
    log('Error fetching organic traffic data:', error);
    return NextResponse.json({ error: 'Failed to fetch organic traffic data' }, { status: 500 });
  }
}