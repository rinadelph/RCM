...
export async function GET(request: Request) {
  ...
  try {
    ...
    const visibilityData = response.data.rows?.map((row: any) => ({
      date: row.keys[0],
    ...
    log('Error fetching search visibility data:', error);
    return NextResponse.json({ error: 'Failed to fetch search visibility data' }, { status: 500 });
  }
}